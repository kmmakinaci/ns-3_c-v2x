...
uint32_t numVeh = 20; //TODO: MERT num of vehicle dependency
int distLevel = 300;
...
BeaconingProtocol::ReceiveBeacon (Ptr<Socket> socket)
{
	...
	while ((packet = socket->RecvFrom (from)))
	{
		if (packet->GetSize () == 0)
		{
			std::cout<<"communication dropped because of packet size < 0 \n\r"<<std::endl;
			break;
		}
...
BeaconingProtocol::SendBeacon ()
{
	...
	if (m_connected != 0)
	{
		...
		Ptr<MobilityModel> mobTx = m_node->GetObject<MobilityModel>();
		Vector posTx =mobTx->GetPosition();
