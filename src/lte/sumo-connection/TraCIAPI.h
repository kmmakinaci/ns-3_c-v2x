...
public:
     ...
     * @exception SocketException if the connection fails
     */
    ...
    /// @name Atomar getter
    /// @{
    int getUnsignedByte(int cmd, int var, const std::string& id, Storage* add = 0);
    int getByte(int cmd, int var, const std::string& id, Storage* add = 0);
    int getInt(int cmd, int var, const std::string& id, Storage* add = 0);
    double getDouble(int cmd, int var, const std::string& id, Storage* add = 0);
    libsumo::TraCIPositionVector getPolygon(int cmd, int var, const std::string& id, Storage* add = 0);
    libsumo::TraCIPosition getPosition(int cmd, int var, const std::string& id, Storage* add = 0);
    libsumo::TraCIPosition getPosition3D(int cmd, int var, const std::string& id, Storage* add = 0);
    std::string getString(int cmd, int var, const std::string& id, Storage* add = 0);
    std::vector<std::string> getStringVector(int cmd, int var, const std::string& id, Storage* add = 0);
    libsumo::TraCIColor getColor(int cmd, int var, const std::string& id, Storage* add = 0);
    /// @}

    const Storage& getCommandStorage() const {
        return myOutput;
    }
...
protected:
     ...
     */
    void createCommand(int cmdID, int varID, const std::string& objID, Storage* add = nullptr) const;
     ...
     */
    int check_commandGetResult(Storage& inMsg, int command, int expectedType = -1, bool ignoreCommandId = false) const;

    bool processGet(int command, int expectedType, bool ignoreCommandId = false);
    bool processSet(int command);
    /// @}

    void readVariableSubscription(int cmdId, Storage& inMsg);
    void readContextSubscription(int cmdId, Storage& inMsg);
    void readVariables(Storage& inMsg, const std::string& objectID, int variableCount, libsumo::SubscriptionResults& into);
...
protected:
    ...
    mutable Storage myOutput;
    /// @brief The reusable input storage
    mutable Storage myInput;
};
