...
namespace ns3
{
	...
	const int sumSocket::lengthLen = 4;
	...
	// ----------------------------------------------------------------------
	sumSocket::
	sumSocket(std::string host, int port)
		: host_( host ),
		port_( port ),
		socket_(-1),
		server_socket_(-1),
        ...
        getFreeSocketPort()
    {
        ...
        if ( bind(sock, (struct sockaddr*) &self, address_len) < 0)
            BailOnSocketError("Socket::getFreeSocketPort() Unable to bind socket");
        // get the assigned port with getsockname
        if ( getsockname(sock, (struct sockaddr*) &self, &address_len) < 0)
            BailOnSocketError("Socket::getFreeSocketPort() Unable to get socket name");
		...
		accept(const bool create)
	{
		...
		if( socket_ >= 0 )
		{
            ...
            if (create) {
            	sumSocket* result = new sumSocket(0);
		...
		set_blocking(bool blocking) 
	{
		...
		if( server_socket_ > 0 )
		{
		    ...
		    if (ioctlsocket(server_socket_, FIONBIO, &NonBlock) == SOCKET_ERROR)
				BailOnSocketError("Socket::set_blocking() Unable to initialize non blocking I/O");
#else
		...
		connect()
	{
		...
		if( !atoaddr( host_.c_str(), address) )
			BailOnSocketError("Socket::connect() @ Invalid network address");
...
#endif
		if( bytesReceived == 0 )
			throw SocketException( "Socket::recvAndCheck @ recv: peer shutdown" );
		if( bytesReceived < 0 )
			BailOnSocketError( "Socket::recvAndCheck @ recv" );
		...
		const
	{
		if (verbose_)
		{
			std::cerr << label << " " << buffer.size() <<  " bytes via Socket: [";
		...
		GetWinsockErrorString(int err) 
	{
		...
		case WSAECONNABORTED:	return "Software caused Socket abort";
