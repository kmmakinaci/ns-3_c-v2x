...
TraCIAPI::connect(const std::string& host, int port) {
    mySocket = new sumSocket(host, port);
    try {
        mySocket->connect();
...
void
TraCIAPI::createCommand(int cmdID, int varID, const std::string& objID, Storage* add) const {
    myOutput.reset();
    // command length
...
TraCIAPI::send_commandSubscribeObjectVariable(int domID, const std::string& objID, double beginTime, double endTime,
    ...
    if (mySocket == nullptr) {
        throw SocketException("Socket is not initialised");
    }
    Storage outMsg;
...
void
TraCIAPI::check_resultState(Storage& inMsg, int command, bool ignoreCommandId, std::string* acknowledgement) const {
...
int
TraCIAPI::check_commandGetResult(Storage& inMsg, int command, int expectedType, bool ignoreCommandId) const {
...
int
TraCIAPI::getUnsignedByte(int cmd, int var, const std::string& id, Storage* add) {
...
int
TraCIAPI::getByte(int cmd, int var, const std::string& id, Storage* add) {
    createCommand(cmd, var, id, add);
...
int
TraCIAPI::getInt(int cmd, int var, const std::string& id, Storage* add) {
    createCommand(cmd, var, id, add);
...
double
TraCIAPI::getDouble(int cmd, int var, const std::string& id, Storage* add) {
    createCommand(cmd, var, id, add);
...
TraCIAPI::getPolygon(int cmd, int var, const std::string& id, Storage* add) {
...
TraCIAPI::getPosition(int cmd, int var, const std::string& id, Storage* add) {
    libsumo::TraCIPosition p;
    createCommand(cmd, var, id, add);
...
libsumo::TraCIPosition
TraCIAPI::getPosition3D(int cmd, int var, const std::string& id, Storage* add) {
    libsumo::TraCIPosition p;
    createCommand(cmd, var, id, add);
...
std::string
TraCIAPI::getString(int cmd, int var, const std::string& id, Storage* add) {
    createCommand(cmd, var, id, add);
...
TraCIAPI::getStringVector(int cmd, int var, const std::string& id, Storage* add) {
...
libsumo::TraCIColor
TraCIAPI::getColor(int cmd, int var, const std::string& id, Storage* add) {
...
void
TraCIAPI::readVariables(Storage& inMsg, const std::string& objectID, int variableCount, libsumo::SubscriptionResults& into) {
...
void
TraCIAPI::readVariableSubscription(int cmdId, Storage& inMsg) {
...
void
TraCIAPI::readContextSubscription(int cmdId, Storage& inMsg) {
